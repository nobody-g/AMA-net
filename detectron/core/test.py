...
get_featmap = False
feattmap_list = ['fpn_res5_2_sum', 'fpn_res4_5_sum', 'fpn_res3_3_sum', 'fpn_res2_2_sum']
def im_detect_all(model, im, box_proposals, timers=None):
    ...
    if get_featmap:
        ...
        for feat_i in feattmap_list:
            feat_i_np = workspace.FetchBlob(core.ScopedName(feat_i)).squeeze()
            np.save(str('/home/densepose_wxh/densepose/tools/vis_demo/')+str(feat_i),feat_i_np)
        data = workspace.FetchBlob(core.ScopedName('data')).squeeze()
        np.save(str('/home/densepose_wxh/densepose/tools/vis_demo/') + str('data'), data)
        np.save(str('/home/densepose_wxh/densepose/tools/vis_demo/')+str('cls_boxes'), cls_boxes)
        np.save(str('/home/densepose_wxh/densepose/tools/vis_demo/') + str('boxes'), boxes)
        np.save(str('/home/densepose_wxh/densepose/tools/vis_demo/') + str('scores'), scores)
    ...
    if cfg.MODEL.BODY_UV_ON and boxes.shape[0] > 0:
        ...
        if not cfg.BODY_UV_RCNN.ONLY_PARTSEG:
            #ext='_inter_5'
            ext = cfg.TEST.EXTRACT_LAYER
            cls_bodys = im_detect_body_uv(model, im_scale, boxes, ext=ext)
            cls_parsings = im_detect_body_uv_onlypartseg(model, im_scale, boxes, im.shape[0], im.shape[1],ext=ext)
            if cfg.MODEL.MASK_ON or cfg.BODY_UV_RCNN.MULTI_ROI:
                cls_personmask = im_detect_mask(model, im_scale, boxes,output_name='person_mask',ext=ext)
                cls_personmask = segm_results(
                    cls_boxes, cls_personmask, boxes, im.shape[0], im.shape[1],
                    output_name='person_mask')
            else:
                cls_personmask = None
        else:
            cls_bodys = None
            cls_parsings = im_detect_body_uv_onlypartseg(model, im_scale, boxes, im.shape[0], im.shape[1])
            if cfg.MODEL.MASK_ON or cfg.BODY_UV_RCNN.MULTI_ROI:
                cls_personmask = im_detect_mask(model, im_scale, boxes, output_name='person_mask')
                cls_personmask = segm_results(
                    cls_boxes, cls_personmask, boxes, im.shape[0], im.shape[1],
                output_name = 'person_mask')
            else:
    ...
    return cls_boxes, cls_segms, cls_keyps, cls_personmask, cls_parsings, cls_bodys
...
def im_detect_mask(model, im_scale, boxes, output_name='mask_fcn_probs',ext=''):
    ...
    """
    if output_name == 'mask_fcn_probs':
        M = cfg.MRCNN.RESOLUTION
    elif output_name == 'person_mask':
    ...
    if cfg.FPN.MULTILEVEL_ROIS:
        _add_multilevel_rois_for_test(inputs, 'body_uv_rois') #'mask_rois')
    ...
    pred_masks = workspace.FetchBlob(
        core.ScopedName(output_name+ext)
    ).squeeze()

    pred_masks_n = []
    if output_name == 'person_mask':
        if pred_masks.ndim == 3:
            pred_masks = np.expand_dims(pred_masks, axis=0)
        for i in range(pred_masks.shape[0]):
            pred_masks_n.append(softmax(pred_masks[i]))
    pred_masks = np.array(pred_masks_n)
...
def segm_results(cls_boxes, masks, ref_boxes, im_h, im_w, output_name='mask_fcn_probs'):
    num_classes = cfg.MODEL.NUM_CLASSES
...
def im_detect_body_uv(model, im_scale, boxes, ext=''):
    ...
    AnnIndex = workspace.FetchBlob(core.ScopedName('AnnIndex'+ext)).squeeze()
    Index_UV = workspace.FetchBlob(core.ScopedName('Index_UV'+ext)).squeeze()
    U_uv = workspace.FetchBlob(core.ScopedName('U_estimated'+ext)).squeeze()
    V_uv = workspace.FetchBlob(core.ScopedName('V_estimated'+ext)).squeeze()
    ''''''
    if (cfg.MODEL.MASK_ON and cfg.MRCNN.DP_CASCADE_MASK_ON) or cfg.BODY_UV_RCNN.MULTI_ROI:
    ...
    for ind, entry in enumerate(boxes):
        ...
        if cfg.MODEL.MASK_ON or cfg.BODY_UV_RCNN.MULTI_ROI:
           ...
           CurIndex_UV = CurIndex_UV * (CurPMaskProb[1] > cfg.BODY_UV_RCNN.SOFT_MASK_THRESH).astype(np.float32)
        else:
...
def im_detect_body_uv_personmask(model, im_scale, boxes, im_h, im_w):
    """Compute body uv predictions."""
    M = cfg.BODY_UV_RCNN.HEATMAP_SIZE
    P = cfg.BODY_UV_RCNN.NUM_PATCHES
    ...
    outputs = []
    #padded_CurAnnIndex = np.zeros((M + 2, M + 2, P+1), dtype=np.float32)
    for ind, entry in enumerate(boxes):
        # Compute ref box width and height
        ...
        #padded_CurAnnIndex[1:-1, 1:-1] = CurAnnIndex
        # Resize p from (HEATMAP_SIZE, HEATMAP_SIZE, c) to (int(bx), int(by), c)
        CurPMask = cv2.resize(CurPMask, (by, bx),interpolation=cv2.INTER_LINEAR)


        # Bring Cur_Preds axes back to CHW
        CurPMask = np.swapaxes(CurPMask, 0, 2)
        CurPMaskProb = softmax(CurPMask)
        CurPMaskProb = (CurPMaskProb[1] > cfg.BODY_UV_RCNN.SOFT_MASK_THRESH).astype(np.float32)
	   #CurAnnIndex = CurAnnIndex * (CurPMaskProb[1] > cfg.BODY_UV_RCNN.SOFT_MASK_THRESH).astype(np.float32)
        im_parsing = np.zeros((im_h, im_w), dtype=np.uint8)
        x_0 = max(entry[0], 0)
        x_1 = min(entry[2], im_w)
        y_0 = max(entry[1], 0)
        y_1 = min(entry[3], im_h)
        im_parsing[y_0:y_1, x_0:x_1] = CurPMaskProb[
            (y_0 - entry[1]):(y_1 - entry[1]),
            (x_0 - entry[0]):(x_1 - entry[0])
        ]
        outputs.append(im_parsing)
...
def im_detect_body_uv_onlypartseg(model, im_scale, boxes, im_h, im_w,ext=''):
    """Compute body uv predictions."""
    M = cfg.BODY_UV_RCNN.HEATMAP_SIZE
    P = cfg.BODY_UV_RCNN.NUM_PATCHES
    ...
    ''''''
    if (cfg.MODEL.MASK_ON and cfg.MRCNN.DP_CASCADE_MASK_ON) or cfg.BODY_UV_RCNN.MULTI_ROI:
       person_mask = workspace.FetchBlob(core.ScopedName('person_mask')+ext).squeeze()
       if person_mask.ndim == 3:
          person_mask = np.expand_dims(person_mask, axis=0)
    # In case of 1
    ...
    for ind, entry in enumerate(boxes):
        ...
        #padded_CurAnnIndex[1:-1, 1:-1] = CurAnnIndex
        # Resize p from (HEATMAP_SIZE, HEATMAP_SIZE, c) to (int(bx), int(by), c)
        CurAnnIndex = cv2.resize(CurAnnIndex, (by, bx),interpolation=cv2.INTER_LINEAR)
        ...
        y_0 = max(entry[1], 0)
        y_1 = min(entry[3], im_h)
        im_parsing[y_0:y_1, x_0:x_1] = CurAnnIndex[
            (y_0 - entry[1]):(y_1 - entry[1]),
            (x_0 - entry[0]):(x_1 - entry[0])
        ]
        outputs.append(im_parsing)
