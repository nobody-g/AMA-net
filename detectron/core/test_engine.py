...
def test_net_on_dataset(
    ...
    if not load_from_tmp:
        ...
        if cfg.VIS:
            vis_wholedataset(dataset_name, proposal_file, output_dir,
                             all_boxes=all_boxes,all_segms=all_segms,all_keyps=all_keyps,
                             all_personmasks=all_personmasks,all_parss=all_parss,all_bodys=all_bodys,
                             img_name=['COCO_val2014_000000464089.jpg'],show_box=False,
            )
    results = task_evaluation.evaluate_all(
        dataset, all_boxes, all_segms, all_keyps, all_personmasks, all_parss, all_bodys, output_dir
    )
    return results
...
def multi_gpu_test_net_on_dataset(
    ...
    all_personmasks = [[] for _ in range(cfg.MODEL.NUM_CLASSES)]
    all_parss = [[] for _ in range(cfg.MODEL.NUM_CLASSES)]
    ...
    for det_data in outputs:
        ...
        all_personmask_batch = det_data['all_personmak']
        all_parss_batch = det_data['all_parss']
        all_parss_txt = det_data['txt_all']
        txt_all.extend(all_parss_txt)
        for cls_idx in range(1, cfg.MODEL.NUM_CLASSES):
            ...
            all_personmasks[cls_idx] += all_personmask_batch[cls_idx]
            all_parss[cls_idx] += all_parss_batch[cls_idx]
    det_file = os.path.join(output_dir, 'detections.pkl')
    cfg_yaml = yaml.dump(cfg)
    if cfg.MODEL.PARSING_ON:
        txt_file = os.path.join(output_dir, 'results.txt')
        with open(txt_file ,'w') as f:
            f.writelines(txt_all)
            f.close()
    save_object(
        dict(
            all_boxes=all_boxes,
        ...
        if 'has_no_densepose' in entry.keys():
            ...
            if cfg.MODEL.PARSING_ON:
                parsing_i_png, txt_result = parsing_utils.parsing2png(
                    cls_boxes_i, cls_parss_i, output_dir, entry['image'], im.shape[:2]
                )
                '''
...
def vis_wholedataset(
    ...
    output_dir,
    ind_range=None,
    all_boxes=None, all_segms=None, all_keyps=None, all_personmasks=None, all_parss=None, all_bodys=None,
    img_name=None, show_box=True,
):
    """Run inference on all images in a dataset or over an index range of images
    in a dataset using a single GPU.
    """
        ...
        if 'has_no_densepose' in entry.keys():
            ...
            if cfg.MODEL.PARSING_ON:
                ...
                uv_parsing_i_png, uv_txt_result = parsing_utils.parsing2png(
                    cls_boxes_i, cls_bodys_i, output_dir, entry['image'], im.shape[:2], uv=True,
                )
                '''
                person_mask_i_png, txt_result_pm = parsing_utils.parsing2png(
                    cls_boxes_i, cls_personmask_i, output_dir, entry['image'], im.shape[:2],flag_pm=True,
...
def empty_results(num_classes, num_images):
    ...
    all_parss = [[[] for _ in range(num_images)] for _ in range(num_classes)]
    all_personmask = [[[] for _ in range(num_images)] for _ in range(num_classes)]
    return all_boxes, all_segms, all_keyps, all_personmask, all_parss, all_bodys
...
def get_ind_results(index, results):
    res_ind = []
    for res_class_i in results:
        res_ind.append(res_class_i[index])
    return res_ind